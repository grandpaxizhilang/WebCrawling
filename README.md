...
- **代码中均有删改，有些代码直接运行会出现报错是正常，也存在网站对反爬进行了更新导致数据不可爬取**
...
- 小红书最新版逆向分析
- 网站对代码进行混淆，我这里进行了解混淆并替换到网站中运行，可正常运行
- 环境检测好像也没有多增加，使用补环境框架只需要改变相关环境参数即可
- 这里js中的data和请求中的data需要一致，不然会请求不会返回数据

- **后续的话就不提供补环境的代码了，代码都是补环境框架的代码，自行使用即可**
...
- 也算是比较简单的vmp了，这里我先插桩相关apply调用，但是发现输出的日志都是没用的信息，所以直接对操作符进行插桩，出来的日志就能看到加密过程了。这里可以先HOOK住Math.random()，这样最后生成的加密参数就不会变化了，可以看还原算法过程是否有错误。

- 加密流程：
  
  ```
  1、首先构造入参，由headers的x-zse-93、接口后缀（去除domain）、cookie中的d_c0 拼接后进行md5加密;
  2、对md5传参进行charCodeAt操作后再放入随机数，生成一个数组;
  3、对这个数组进行加密再生成一个新的数组;
  4、对新的数组遍历进行charAt操作生成单个字符，拼接生成加密参数;
  ```
  
- x-zse-93纯算：zse_96.js
...
- token值进行跟栈也就进入一个js文件中，扫一眼就是标准的vmp。直接插桩看日志，整体的加密流程：
  ```
  1、获取时间戳、ua、cookie中的一个参数、随机生成的uuid拼接成一个字符串后进行魔改的md5加密;
  2、拿md5加密的参数和其他浏览器环境组成一个数组;
  3、对这个数组每个参数的头和尾生成长度为3的字符串后使用" # "进行拼接成一个字符串;
  4、对这个字符串进行charCodeAt操作生成一个数组;
  5、最后就是通过这个数组进行一些运算和charAt等操作生成单个字符，拼接后就是token值了;
  ```
...
- 跟栈找到加密位置，发现是jsvmp并且没混淆，那就直接上手插桩打印日志然后分析，加密流程：
  ```
  1、生成随机的32位字符串，然后拼接时间戳和url进行md5加密;
  2、构造json数据，里面有加载时间，上面生成的时间戳、md5加密值以及有关指纹的参数fingerprint，但是这个fingerprint可以置为空;
  3、最后就是对这个json转字符串后进行魔改的RC4加密;
  ```
...
- 美团系 **mtgsig** 参数纯算，版本: 1.2
- 这里我验证的是大众点评店铺里面的评论接口（别问为什么不是美团，因为之前弄美团把自己的号给整出问题了😐）
- 话不多说，直接跟栈找到加密位置，发现参数生成是运行H5guard.js文件后返回的。进去看一眼代码，发现混淆了，简单解一下混淆扔到网站进行文件替换运行，然后找到jsvmp入口位置直接插桩分析日志，下面是各个参数的加密流程：

```
a1: 版本号——可固定;
a2: 时间戳;
a3: cookie里面的一个值;
a5: （a6前十位 + 时间戳 + 随机数）进行加密生成;
a6: 通过66位数组和一些环境参数进行aes和base64加密生成;
a8:	也是通过66位数组以及生成a5过程中的一个数组进行toString(16)计算生成;
a9: 版本号 + 66位数组中的一个参数;
a10: 随机的2位字符串;
x0: 固定值;
d1: 上面全部参数合并和（请求方式 + 请求api + 请求参数） 进行加密生成;
```

- 这里66位数组是随机生成的，这个数组非常重要，所有参数都围绕着这个数组进行加密。但是我这里是把这个数组固定了。不知道为什么我随机生成的66位数组最后生成的mtgsig进行请求一直不过，一固定就没问题了😢
- 有关aes和base64加密在H5guard.js上都有，直接扣也行，自己用加密库来弄也行。这里我怕有魔改就直接扣代码了
...
- 抖音 **a_bogus** 参数纯算，版本: V1.0.1.19_fix

- 加密位置自行寻找，网上随便一搜都有教程。这里定位到是在函数d()里面生成的，看一眼就能看出是jsvmp了，并且上了控制流混淆，不太好看且不好插桩，这里就先进行解混淆再插桩看日志。总的来说难度不是很难，就是加密流程较为繁琐和写解混淆插件花了点时间，整体加密流程如下：

  ```
  1、对请求参数params + "dhzx" 进行两次sm3加密，生成32位数组;
  2、对 "dhzx" 进行两次sm3加密，生成32位数组;
  3、对ua进行魔改RC4加密,然后进行charCodeAt操作后再进行sm3加密;
  4、通过浏览器环境参数和时间戳参数进行charCodeAt操作，获取两个数组;
  5、将前面四步生成的数组和时间戳生成50位数组;
  6、生成随机的八位数组，通过这个随机的八位数组和50位数组进行异或操作生成一位数组;
  7、组合第四、五、六步的数组后再进行一些计算操作获取新的数组;
  8、对第七步获取的数组进行charCodeAt操作，然后再进行魔改RC4加密;
  9、随机生成四位数组并charCodeAt操作 + 第八步的字符串;
  10、对第九步的字符串进行魔改的BASE64加密后生成a_bogus;
  ```

- 主要注意的是50位数组的生成，如果只是插桩apply和一些运算操作是一定不知道50位数组是怎么生成的。这里只要在case 54 和case 74 进行插桩，查看数组的入栈和取值，一切都会清晰明了
- 还有另一个参数msToken，这个参数是请求接口获取，自行寻找
- 解混淆后的代码：decode.js
- 请求流程：main.js
