...
Document.prototype.cookie = 'abRequestId=535ba081-a892-5fe3-a2d9-801947515007; xsecappid=xhs-pc-web; a1=18ec200d304gs0cnydlf5msz92tiuqzemu2l4ef1h50000126106; webId=e4faf058594573d94c3f5a4339ba310a; gid=yYdSJ88fdY6YyYdSJ88fq7di84kM8SIvf1i2Jyx6MujJ912837Aud4888yJKy8K84WWyjSK2; webBuild=4.11.0; unread={%22ub%22:%2266077eac0000000014004ada%22%2C%22ue%22:%2265f3a476000000000d00f83f%22%2C%22uc%22:14}; websectiga=82e85efc5500b609ac1166aaf086ff8aa4261153a448ef0be5b17417e4512f28; sec_poison_id=396f8610-c6c4-40ad-9ea5-ca3de612dfa1'
...
Document.prototype.createElement = function createElement(tagName){
    ...
    if((tagName.toString().indexOf('object') != -1) || (tagName.toString() == '')){
        //这里有问题，应该是出现报错，但是这里报错会让代码停止，报错的类型是new DOMException(''),但是这个接口还未实现
        ...
        console.log("Failed to execute 'createElement' on 'Document': The tag name provided ('"+tagName.toString()+"') is not a valid name.")
    }else{
...
var RegExp = new Proxy(RegExp,{
    construct(target, argArray) {
        ...
        if(catchvm.memory.tegexp_count == 1){
            console.log('这里用正则检测了是否有vm关键字 ——> RegExp(["vm|bootstrapNodeJSCore|tryModuleLoad|evalmachine|runInContext", "g"])')
            return new target('["|bootstrapNodeJSCore|tryModuleLoad|evalmachine|runInContext", "g"]')
        }

        if(catchvm.memory.tegexp_count == 2){
            console.log(arguments)
            debugger;
        }
